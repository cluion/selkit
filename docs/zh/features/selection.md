...
> 父節點可選。`treeCascade: true`（預設，僅多選）下，選父會勾選所有子孫葉，部分勾選時父
> 顯示**半選**狀態；設 `treeCascade: false` 為獨立選取。預設全部展開。搜尋會過濾樹並自動展開
> 命中節點的祖先（清空查詢恢復）。樹狀模式支援虛擬捲動（設 `virtualScroll: true`，與扁平
> 清單一致）；收合與搜尋會重算可視窗格。
