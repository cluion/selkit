...
createSelkitDom('#big', {
  ...
  virtualScroll: true,
  itemHeight: 38, // px, must match rendered row height
})
```
...
`itemHeight` defaults to `38`, which matches the base theme's option height. Set
...
createSelkitDom('#big', {
  ...
  virtualScroll: true,
  itemHeight: 38,  // option row height
